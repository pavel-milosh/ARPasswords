...
async def _key(message: Message, state: FSMContext, **kwargs) -> None:
    ...
    button_text: str = (await local("common", "change_?")).format(parameter=local("common", "key"))
    text: str = (await local("c_key", "initial")).format(key=kwargs["key"], hours=hours)
    keyboard: InlineKeyboardMarkup = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=button_text, callback_data="change_key")],
...
async def _key_set(message: Message, state: FSMContext) -> None:
    ...
    await bot_message.edit_text(await local("c_key", "done"))
    await state.clear()
